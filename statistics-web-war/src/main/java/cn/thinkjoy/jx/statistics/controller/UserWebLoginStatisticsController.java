 ...
 */
@Controller
@RequestMapping("/statistics/webUserLogin")
public class UserWebLoginStatisticsController {

    private static Logger LOGGER = LoggerFactory.getLogger(UserWebLoginStatisticsController.class);




    @Autowired
    private IClassDayWebLoginService classDayWebLoginService;

    @Autowired
    private IUserDayWebLoginService userDayWebLoginService;
...
/*        if(list!=null && list.size()==1 && list.get(0)==-1){//判断是否是公司管理员
        ...
        List<StatisticsPojo> statisticsPojoList = classDayWebLoginService.queryByTime(dataMap);
...
/*        if(list!=null && list.size()==1 && list.get(0)==-1){//判断是否是公司管理员
        ...
        Page<StatisticsPojo> page = classDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "areaId", SqlOrderEnum.ASC.getAction(), Constants.BETWEEN_DATEDAY_QUERY_AREA_NUM);
...
/*        if(list!=null && list.size()==1 && list.get(0)==-1){//判断是否是公司管理员
        ...
        Page<StatisticsPojo> page = classDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "schoolId", SqlOrderEnum.ASC.getAction(), Constants.WHERR_AREAID_QUERY_SCHOOL_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public Page<StatisticsPojo> queryClassLoginUserNum(HttpServletRequest request) {
        ...
        Page<StatisticsPojo> page = classDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "classId", SqlOrderEnum.ASC.getAction(), Constants.WHERR_SCHOOLID_QUERY_CLASS_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
   ...
   /**************************************/
    ...
    public List<StatisticsPojo> queryUserLoginNum(HttpServletRequest request) {
        ...
        dataMap.put("dateStart",dateStart);
        dataMap.put("dateEnd", dateEnd);
        List<StatisticsPojo> statisticsPojoList = userDayWebLoginService.queryByTime(dataMap);
        if(statisticsPojoList!=null && statisticsPojoList.size()>0){
            return  statisticsPojoList;
        }else{
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
    ...
    public Page<StatisticsPojo> queryAreaUserLoginNum(HttpServletRequest request) {
        ...
        dataMap.put("dateDay", dateDay);
        Page<StatisticsPojo> page = userDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "areaId", SqlOrderEnum.ASC.getAction(), Constants.BETWEEN_DATEDAY_QUERY_AREA_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public Page<StatisticsPojo> querySchoolUserLoginNum(HttpServletRequest request) {
        ...
        dataMap.put("dateDay",dateDay);
        dataMap.put("areaId", Long.valueOf(areaId));
        Page<StatisticsPojo> page = userDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "schoolId", SqlOrderEnum.ASC.getAction(), Constants.WHERR_AREAID_QUERY_SCHOOL_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public Page<StatisticsPojo> queryClassUserLoginNum(HttpServletRequest request) {
        ...
        dataMap.put("dateDay",dateDay);
        dataMap.put("schoolId",  Long.valueOf(schoolId));
        Page<StatisticsPojo> page = userDayWebLoginService.queryByPageTime(dataMap, (currentPageNo - 1) * pageSize, pageSize, "classId", SqlOrderEnum.ASC.getAction(), Constants.WHERR_SCHOOLID_QUERY_CLASS_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public List<StatisticsPojo>  queryUserLoginDateNum(HttpServletRequest request) throws ParseException {
        ...
        if (StringUtils.isBlank(dateDay)){
            ...
            List<StatisticsPojo> list = userDayWebLoginService.queryLoginUserCountByTime(dataMap,range,dateDay);
    ...
    public Page<StatisticsPojo> queryAreaUserLoginDateNum(HttpServletRequest request) throws ParseException {
        ...
        Page<StatisticsPojo> page = userDayWebLoginService.queryLoginUserCountByPageTime(dataMap,dateDay,start,end,(currentPageNo - 1) * pageSize, pageSize, Constants.BETWEEN_DATEDAY_QUERY_AREA_NUM);
        if(page == null || page.getCount()==0 ){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public Page<StatisticsPojo> querySchoolUserLoginDateNum(HttpServletRequest request) throws ParseException {
        ...
        Page<StatisticsPojo> page = userDayWebLoginService.queryLoginUserCountByPageTime(dataMap, dateDay,start, end, (currentPageNo - 1) * pageSize, pageSize,Constants.WHERR_AREAID_QUERY_SCHOOL_NUM,Integer.parseInt(areaId));
        if (page==null||page.getCount()==0){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
    ...
    public Page<StatisticsPojo> queryClassUserLoginDateNum(HttpServletRequest request) throws ParseException {
        ...
        //在方法中增加了一个空map集合
        Page<StatisticsPojo> page = userDayWebLoginService.queryLoginUserCountByPageTime(new HashMap<String, Object>() ,dateDay,start,end, (currentPageNo - 1) * pageSize, pageSize,Constants.WHERR_SCHOOLID_QUERY_CLASS_NUM,Integer.parseInt(schoolId));
        if (page==null||page.getCount()==0){
            throw new BizException(ERRORCODE.NO_MESSAGE.getCode(), ERRORCODE.NO_MESSAGE.getMessage());
        }
        return page;
