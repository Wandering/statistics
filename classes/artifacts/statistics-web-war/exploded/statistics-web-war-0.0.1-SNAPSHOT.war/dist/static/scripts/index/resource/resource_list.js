 ...
 */
define('static/scripts/index/resource/resource_list', ['sea-modules/bootstrap/bootstrap', 'sea-modules/jquery/cookie/jquery.cookie', 'static/scripts/index/message', 'static/scripts/index/resource/resource_form', 'static/scripts/index/datatable', 'sea-modules/jquery/dialog/jquery.dialog'], function(require, exports, module) {
    ...
    module.exports = function(parentCode, treeObj) {
        ...
        var vali = require('static/scripts/index/resource/resource_form');
        ...
        var TableInstance = Table({
            ...
            sAjaxSource: '/statistics/resource/getResourcePage?menuCode=' + parentCode.id + '&token=' + token
        });
        TableInstance.init();
        ...
        var addOrUpdateDepartment = function(formArry, succCallback, id) {
            ...
            $.ajax({
                type: 'post',
                url: '/statistics/resource/addOrEditResource?token=' + token,
        ...
        var ButtonEvent = {
            ...
            update: function(elementId) {
                ...
                $('#' + elementId).on('click', function(e) {
                    ...
                    var anSelected = TableInstance.selectedRecord();
                    if (anSelected.length === 0) {
                        return;
                    }
                    var aData = anSelected[0];

                    $.get('/statistics/resource/getResourcesDetail?resourceId=' + aData.resourceId + '&token=' + token, function(data) {
                        if ('0000000' === data.rtnCode) {
                            $.get('../tmpl/resource/resource_form.html', function(tmpl) {
                                require('sea-modules/jquery/dialog/jquery.dialog');
                                $("#add_resource").dialog({
                    ...
                    if (anSelected.length !== 0) {
                        ...
                        message({
                            ...
                            clickHandle: function() {
                                $.ajax({
                                    type: 'post',
                                    url: '/statistics/resource/delResources?resourceId=' + aData.resourceId + '&token=' + token,
                                    dataType: 'json',
                                    success: function(data) {
                                        if ('0000000' === data.rtnCode) {
                                            tableObj.fnDraw();
