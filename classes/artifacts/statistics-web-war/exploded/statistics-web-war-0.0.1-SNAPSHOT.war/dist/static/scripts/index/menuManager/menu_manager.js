 ...
 */
define('static/scripts/index/menuManager/menu_manager', ['sea-modules/jquery/cookie/jquery.cookie', 'static/scripts/index/common/urlConfig'], function(require, exports, module) {
	/**
	 *
	 * @param succ  有数据处理方式
	...
	module.exports = function(options) {
		...
		var getGridByTree = function(selectedId) {
			require.async(['static/scripts/index/menuManager/menu_list'], function(module) {
				module(selectedId, function(options) {
					//增加节点
					if ('add' === options.type) {
						var parentNode = gTree.treeObj.getNodeByParam('id', selectedId, null);
						gTree.treeObj.addNodes(parentNode, options.obj);
					} else if ('update' === options.type) {
						//修改节点
						var updateNode = gTree.treeObj.getNodeByParam('id', options.obj.id, null);
						updateNode.name = options.obj.name;
						gTree.treeObj.updateNode(updateNode);
					} else if ('delete' === options.type) {
						//删除节点
						var deleteNode = gTree.treeObj.getNodeByParam('id', options.id, null);
						gTree.treeObj.removeNode(deleteNode);
					}
				});
			});
		};
