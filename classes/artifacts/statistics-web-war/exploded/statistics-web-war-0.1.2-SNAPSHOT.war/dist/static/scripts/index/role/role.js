 ...
 */
define('static/scripts/index/role/role', ['sea-modules/bootstrap/bootstrap', 'sea-modules/jquery/cookie/jquery.cookie', 'sea-modules/jquery/dialog/jquery.dialog', 'static/scripts/index/message', 'static/scripts/index/datatable', 'static/scripts/index/tree'], function(require, exports, module) {
    module.exports = function() {
        ...
        var TableInstance = Table({
            ...
            sAjaxSource: '/statistics/role/queryRoles?token=' + token
        });
        TableInstance.init();
        ...
        function renderResourceTree(menuCodeList, resourceList) {
            $.get('/statistics/menu/getMenuTree?token=' + token + '&hasResource=true&systemCode=' + systemCode, function(data) {
                if ('0000000' === data.rtnCode) {
                    var nodes = data.bizData.treeBeanList;
                    if (menuCodeList && resourceList) {
                        addCheckedStatus(nodes, menuCodeList, resourceList);
                    }
                    Tree = require('static/scripts/index/tree');
                    Tree.init({
                        id: 'tree_resource',
                        check: true,
                        nodes: nodes
        ...
        function addOrUpdateRole(id, roleCode) {
            ...
            if (!ajaxFlag) {
                ...
                $.ajax({
                    type: 'post',
                    url: '/statistics/role/addOrEditRoles?token=' + token,
        ...
        var ButtonEvent = {
            ...
            update: function(elementId) {
                ...
                $('#' + elementId).on('click', function(e) {
                    ...
                    $.get('/statistics/role/getRoles?id=' + aData.id + '&token=' + token, function(data) {
                        if ('0000000' === data.rtnCode) {
                            $.get('../tmpl/role/role.tmpl', function(tmpl) {
                                $("#add_role").dialog({
                                    title: "修改角色",
                ...
                $('#' + elementId).on('click', function(e) {
                    ...
                    if (anSelected.length !== 0) {
                        ...
                        message({
                            ...
                            type: 'alert',
                            clickHandle: function() {
                                $.get('/statistics/role/delRoles?rolesId=' + aData.id + '&token=' + token, function(data) {
                                    if ('0000000' === data.rtnCode) {
                                        tableObj.fnDraw();
                                    } else {
                                        message({
