define('static/scripts/index/menuManager/menu_list', ['sea-modules/bootstrap/bootstrap', 'sea-modules/jquery/cookie/jquery.cookie', 'sea-modules/jquery/dialog/jquery.dialog', 'static/scripts/index/message', 'static/scripts/index/datatable', 'static/scripts/index/common/urlConfig', 'static/scripts/index/menuManager/menu_form'], function(require, exports, module) {
	module.exports = function(productCode, treeCallback) {
		...
		var TableInstance = Table({
			...
			sAjaxSource: UrlConfig.queryPageMenu + '&menuCode=' + productCode
		});
		TableInstance.init();
		...
		var ButtonEvent = {
					...
					$.get('../tmpl/menu/menu_dialog.tmpl', function(tmpl) {
						$("#add_menu").dialog({
							...
							buttons: [{
								...
								click: function() {
									var vali = require('static/scripts/index/menuManager/menu_form');
			...
			update: function(elementId) {
				...
				$('#' + elementId).on('click', function(e) {
					...
					$.get(UrlConfig.getMenuDetail + '&menuId=' + aData.id, function(ret) {
				...
				$('#' + elementId).on('click', function(e) {
					...
					message({
						...
						clickHandle: function() {
							$.ajax({
								type: 'post',
								url: UrlConfig.delMenu + '&menuId=' + aData.id,
								dataType: 'json',
								success: function(data) {
									if ('0000000' === data.rtnCode) {
										tableObj.fnDraw();
